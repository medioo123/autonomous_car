...
def horizontal_flip_regression(img, angle):
...
def change_perspective(img,i):
    """Change perspective image.
    Img: Input image to transform in Numpy array.
    Angle: Corresponding label. Must be a 5-elements Numpy Array.
    """    
    rows,cols,ch = img.shape 
    pts1 = np.float32([[0,0],[cols,0],[20*i,rows],[cols-20*i,rows]])
    pts2 = np.float32([[0,0],[cols,0],[0,rows],[cols,rows]]) 
    M = cv2.getPerspectiveTransform(pts1,pts2)
    dst = cv2.warpPerspective(img,M,(cols,rows))
    return dst

def change_perspective2(img,i):
    """Change perspective image.
    Img: Input image to transform in Numpy array.
    Angle: Corresponding label. Must be a 5-elements Numpy Array.
    """    
    rows,cols,ch = img.shape 
    pts1 = np.float32([[20*i,0],[cols-20*i,0],[0,rows],[cols,rows]])
    pts2 = np.float32([[0,0],[cols,0],[0,rows],[cols,rows]]) 
    M = cv2.getPerspectiveTransform(pts1,pts2)
    dst = cv2.warpPerspective(img,M,(cols,rows))
    return dst
...
def generate_horizontal_flip_regression(X, Y, proportion=0.25):
    '''Image augmentation by lowering the gamma (darker images).
    X: Numpy array of training images in channels-last format.
    ...
    for index in tqdm(indexes):
        # Apply the desired transformation
        im, angle = horizontal_flip_regression(X[index], Y[index])
        Y_aug.append(angle)
        X_aug.append(im)
...
def generate_change_perspective(X, Y, proportion=0.25):
    '''Image augmentation by lowering the gamma (darker images).
    X: Numpy array of training images in channels-last format.
    Y: Labels MUST be a 5-elements array.
    proportion: percentage of elements to be generated (25% of total by default).
    ...
    for index in tqdm(indexes):
        for i in range(1,4):
            # Apply the desired transformation
            im= change_perspective(X[index], i)
            Y_aug.append(Y[index])
            X_aug.append(im)
    return X_aug, Y_aug

def generate_change_perspective2(X, Y, proportion=0.25):
    '''Image augmentation by lowering the gamma (darker images).
    X: Numpy array of training images in channels-last format.
    Y: Labels MUST be a 5-elements array.
    proportion: percentage of elements to be generated (25% of total by default).
    ...
    for index in tqdm(indexes):
        for i in range(1,4):
            # Apply the desired transformation
            im= change_perspective2(X[index], i)
            Y_aug.append(Y[index])
            X_aug.append(im)
    return X_aug, Y_aug
